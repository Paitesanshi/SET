...
CHATROOM_TEMPLATE = """
Resource Exchange Environment
============================

Round: {turn_idx}/{total_round}
...
Trading Context
--------------
- Resource production occurs each round and everyone will get one type of resource
- Agreements can be honored or betrayed
- Maximize total resource value after {total_round} rounds
- Stay aligned with personal values and personality traits

Generate your next statement focusing on resource exchange proposals, agreements, or reflections on past trades.
"""
...
class SERoom(ChatRoom):
    ...
    def describe(self, agent_name: str, **kwargs: Any) -> str:
        ...
        # survival_summary = "Resource Status:\n"
        # for member in self.members:
        #     if hasattr(member, 'resources'):
        #         status = "Healthy" if min(member.resources.values()) > 5 else "At Risk" if min(member.resources.values()) > 0 else "Starved"
        #         survival_summary += f"{member.name}: {status}\n"
        ...
        # Format history to highlight proposals and outcomes
        formatted_history = []
        for msg in self.get_history(agent_name):
            # Could add tags/formatting here to highlight proposals vs outcomes
            formatted_history.append(f"{msg.name}: {msg.content}")
